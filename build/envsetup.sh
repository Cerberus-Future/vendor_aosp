...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to Cerberus Gerrit Review.
- pixelrebase:     Rebase a Gerrit change and push it again.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for Cerberus Github.
...
- repopick:        Utility to fetch changes from Cerberus Gerrit.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Cerberus-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop org.cerberus.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/Cerberus/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop org.cerberus.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelgerrit() {
    ...
    local user=`git config --get review.gerrit.cerberus.org.username`
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Cerberus Gerrit Rebase Usage: "
    ...
    git fetch "http://gerrit.cerberus.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop org.cerberus.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
